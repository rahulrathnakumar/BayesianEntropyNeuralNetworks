...
smooth_l1_loss = nn.SmoothL1Loss(reduction='sum', beta = 0.2)
...
def value_constraint_loss(eta, N, model, gpu_id, y_test, funcs, *args, **kwargs):
    ...
    for i, func in enumerate(funcs):
        ...
        # y_hat = I1/I2
        cons_loss += eta[i]*smooth_l1_loss(g, y_test[i])
    return cons_loss
...
# eta for the BaseCVAE is just a tensor
eta = torch.tensor([1.0, 1.0], requires_grad = True, device = gpu_id)
# eta = [torch.tensor(0.5, requires_grad = True, device = gpu_id), torch.tensor(0.5, requires_grad = True, device = gpu_id)]
...
for epoch in range(num_epochs):
    ...
    for batch in dataloader['train']:
        ...
        constraint_loss = value_constraint_loss(eta, N, model, gpu_id, y_test, funcs = constraint_functions)
